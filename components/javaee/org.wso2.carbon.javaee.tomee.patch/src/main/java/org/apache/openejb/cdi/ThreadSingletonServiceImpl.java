...
public class ThreadSingletonServiceImpl implements ThreadSingletonService {
	...
	public static final Logger logger = Logger.getInstance(LogCategory.OPENEJB_STARTUP, org.apache.openejb.cdi.ThreadSingletonServiceImpl.class);
	...
	public void initialize(final StartupObject startupObject) {
		...
		// ############ START - WSO2 PATCH ############
		// OpenWebBeansConfiguration#parseConfiguration method don't read the openwebbeans.properties to some issue
		// read the file by adding "/" to the actual path
		//Read openwebbeans.properties file and add them to properties
		Properties newConfigProperties = PropertyLoader.getProperties("/META-INF/openwebbeans/openwebbeans.properties");
		properties.putAll(newConfigProperties);
		properties.put("org.apache.webbeans.spi.ApplicationBoundaryService", "org.apache.webbeans.corespi.se.DefaultApplicationBoundaryService");
		// ############ END - WSO2 PATCH ############
		...
		if (oldClassLoader != org.apache.openejb.cdi.ThreadSingletonServiceImpl.class.getClassLoader() && org.apache.openejb.cdi.ThreadSingletonServiceImpl.class.getClassLoader() != oldClassLoader.getParent()) {
			cl = new MultipleClassLoader(oldClassLoader, org.apache.openejb.cdi.ThreadSingletonServiceImpl.class.getClassLoader());
		} else {
			cl = oldClassLoader;
		}
